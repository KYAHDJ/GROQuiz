...
export default function PowerupBar() {
  ...
  return (
    <div className="w-full bg-[#151021]/90 backdrop-blur-sm border-b border-[#251C33] sticky top-0 z-30">
      <div className="max-w-3xl mx-auto px-4 py-2.5 flex items-center gap-3 overflow-x-auto text-sm no-scrollbar">
        ...
        <div className="w-px h-4 bg-[#3A2E50]" />
        ...
        <div className="flex items-center gap-1.5 text-[#B8A9C8] whitespace-nowrap">
          <Shield size={14} />
          <span>
            {stats.correct}/{stats.answered}
          </span>
          {stats.answered > 0 && (
            <span className="text-xs text-[#8D7FA0]">{accuracy}%</span>
          )}
        </div>
        ...
        {mode === "hard" ? (
          ...
          />
          <PowerupBadge
            icon={<Clock size={13} className="text-fuchsia-300" />}
